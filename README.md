...
Download GitHub repository and execute the script 0.haplomagic.R on the example data:
```
Rscript 0.haplomagic.R example 1 3 imputeTHonly correctFalseHom 10000
```
For more information, please read the following manual.

## User's manual

For running haploMAGIC, install the scripts in the same directory as the input. haploMAGIC requires two files as input:
...
```
haploMAGIC arguments explained:
